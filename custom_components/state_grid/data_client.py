...
class StateGridDataClient:
	...
	async def __get_door_mouth_bill(F,door_account,monthBill):
		S='pointList';K=monthBill;J='amtList';I='readList';H='quantity';G='priceName';B=door_account;L=datetime.datetime.strptime(K[_AN],'%Y%m');T=f"{L.year}-{L.month:02d}";U={_A:{_I:configuration[_V][_I],_D:configuration[_V][_D],_T:configuration[_V][_T],_e:configuration[_V][_e],_q:B[_g],_d:B[_U],_Y:B[_Y],_Av:T,_AV:B[_U],_AL:B[_AW],_AI:F.userInfo[_R],_N:'',_L:'',_A4:F.userInfo[_AB],_A3:F.userInfo[_R]},_C:configuration[_V][_C],_E:configuration[_V][_E],_S:B[_U]};C=await F.__fetch(get_door_ladder_api,U);W=F.handle_request_result_message(_Aw,C)
		if _H in C and C[_H]==1 and _A in C and _u in C[_A]:
			D=C[_A][_u][0]
			if I in D:M=D[I][0]
			else:M=D[S][0][I][0]
			V=catchFloat(M,'activeCount')
			if J in D:N=D[J]
			else:N=D[S][0]['amtGroupList'][0][J]
			O=0;P=0;Q=0;R=0
			for A in N:
				if A[G]=='峰':O=catchFloat(A,H)
				elif A[G]=='谷':P=catchFloat(A,H)
				elif A[G]=='平':Q=catchFloat(A,H)
				elif A[G]=='尖':R=catchFloat(A,H)
